...
def refresh_access_token():
    ...
    try:
        ...
        logger.info(f"Token refreshed successfully. Expires at: {credentials.expiry.isoformat()}")
...
def create_kafka_producer():
    ...
    if not all([KAFKA_BOOTSTRAP_SERVERS, KAFKA_API_KEY, KAFKA_API_SECRET, KAFKA_TOPIC]):
        logger.warning("Missing Kafka environment variables. Kafka production disabled.") 
        return None
    ...
    with grpc.secure_channel("dns:///youtube.googleapis.com:443", creds) as channel:
        ...
        while True:
            ...
            # Build the request message using the generated Protobuf class
                ...
                if e.code() == grpc.StatusCode.UNAUTHENTICATED:
                    logger.error("Authentication failed. Check if your OAuth token is expired or invalid.")
                    break 

                elif e.code() == grpc.StatusCode.UNAVAILABLE or e.code() == grpc.StatusCode.INTERNAL:
                    logger.error("Connection issue. Attempting reconnect in 5 seconds...")
                    time.sleep(5)
                    continue
        ...
        if producer:
            ...
            if remaining_messages > 0:
                logger.error(f"WARNING: {remaining_messages} messages still in queue after flush timeout. Data loss possible.") 
            else:
...
def index():
    ...
    if request.method == 'POST':
        ...
        if 'message' in envelope and 'data' in envelope['message']:
            ...
            try:
                ...
                if video_id:
                    ...
                    if live_chat_id:
                        logger.info(f"Live Chat ID {live_chat_id} found. Starting ingestion thread.")
                        
                        ingestion_thread = threading.Thread(target=youtube_ingest, args=(live_chat_id,))
                        ...
                        logger.warning(f"Could not find active live chat ID for video {video_id}. Returning 200 to avoid Pub/Sub retry.")
        ...
        logger.info("Received POST request with valid JSON but incorrect Pub/Sub structure.")
